...
std::vector<std::string> C_IPportlist;//IP:port
std::vector<std::string> C_IPlist;//IP
std::vector<int> C_Portlist;//port
//存储加入的cache IP\port\IP:port列表
std::vector<std::string> IPportlist;//IP:port
std::vector<std::string> IPlist;//IP
std::vector<int> Portlist;//port
std::vector<std::string> shutipnow,shutportnow;//存放缩容时需要关闭的ip和port数组
int num4=0;//主动缩容shutportnow的长度
int num5=0;//每个断开的port都可能出现即使没有重连依然出现一个心跳包，所以每检测到一个就计数，直到等于num4
std::string shutip,shutport;//主动缩容的cache IP port
std::string psshutip,psshutport;//被动缩容的cache IP port

//存储心跳包里面的ip和port
std::string heartip,heartport;
//port-0/1，发送后为1，否则为0，以判断是否为所有的cache都发送了新的port
std::unordered_map<std::string, int> cachemap;//扩容的时候向其他cache发消息
std::unordered_map<std::string, int> cachemapshut;//缩容的时候向其他cache发消息,否则置1出现问题
//port-0/1，发送后为1，否则为0，以判断是否为所有的client都发送了新的port
//unordered_map<string, int> clientmap;
//被动缩容
std::unordered_map<std::string, int> cachestatemap;//新开线程，检测Portlist里面每个PORT对于的心跳包计数是否一直在加
std::unordered_map<std::string, int> precachestatemap;//新开线程，检测Portlist里面每个PORT对于的心跳包计数是否一直在加
...
/////////////////////////////////////////
//master容灾
std::string masterip,masterport;
std::unordered_map<std::string, int> masterupdatemap;//向所有的cache发送备用master的IP:port

std::unordered_map<std::string, int> masterstatemap;//新开线程，检测Portlist里面每个PORT对于的心跳包计数是否一直在加
std::unordered_map<std::string, int> premasterstatemap;//新开线程，检测Portlist里面每个PORT对于的心跳包计数是否一直在加
                ...
                if(iterr!=premasterstatemap.end())
                {
                    if(iter->second <= iterr->second)   // second 方法得到的是map 中的value
                    {
                        ...
                        for(int i=0;i<IPportlist.size();i++)
                        {
                            ...
                            std::string buf1 = refreshmaster().dump();
                            buf1+="\0";
                            int n=0;
                            ...
                            if(n>0){
                                std::cout<<IPportlist[i]<<":"<<buf1<<std::endl;
                            }
                            Close(cfd);
                        }
                        //delayms(500);
...
void* testallshut(void *arg1)
{
    ...
    while(1)
    {
        ...
        {
            int shutflag=0;
            std::unordered_map<std::string, int> precacheshutmap;
            if(cachestatemap.size()>0)
            {
